...
<cache type="com.qitoon.framework.core.dao.MybatisRedisCache" />
	...
	<select id="findList"  parameterType="${package}.entity.${className?cap_first}" resultMap="BaseResultMap">
	     select <include refid="baseColumns"/> from ${tableName} where 1=1 <include refid="whereSql"/>
	        <if test="startCreateTime != null and endCreateTime!=null ">
                and create_time between ${r'#{startCreateTime}'}  and ${r'#{endCreateTime}'}
            </if>
             <if test="startUpdateTime != null and endUpdateTime!=null ">
                 and update_time between ${r'#{startUpdateTime}'}  and ${r'#{endUpdateTime}'}
            </if>
            <if test="startDeleteTime != null and endDeleteTime!=null ">
                 and delete_time between ${r'#{startDeleteTime}'}  and ${r'#{endDeleteTime}'}
            </if>

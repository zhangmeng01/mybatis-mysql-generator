 ...
 * 自动代码生成工具操作类
 */
public class GenDbCreateCodeForIntro {

	private static Logger logger = LoggerFactory.getLogger(GenDbCreateCodeForIntro.class);
	// 项目的命名空间
	private static final String MODULE = "intro";
	...
	// 指定模版地址
	private static final String DIRCTORY_PATH = "E:\\workspace\\GIT\\xwtoon\\";
	private static final String TEMPLETE_PATH = DIRCTORY_PATH
			+ "xwtoon-tools\\src\\main\\resources\\generator\\introtemplates";
	...
	private static final String CONTROLLER_PATH = DIRCTORY_PATH
			+ "xwtoon-app\\src\\main\\java\\com\\syswin\\xwtoon\\app\\controller\\intro\\";

	private static final String ENTITY_PATH = DIRCTORY_PATH
			+ "xwtoon-dao\\src\\main\\java\\com\\syswin\\xwtoon\\entity\\intro\\";
	private static final String DAO_PATH = DIRCTORY_PATH
			+ "xwtoon-dao\\src\\main\\java\\com\\syswin\\xwtoon\\dao\\intro\\";
	private static final String MAPPER_PATH = DIRCTORY_PATH + "xwtoon-dao\\src\\main\\resources\\mappings\\intro\\";

	private static final String SERVICEIMPL_PATH = DIRCTORY_PATH
			+ "xwtoon-app-service\\src\\main\\java\\com\\syswin\\xwtoon\\app\\service\\impl\\intro\\";
	private static final String SERVICE_PATH = DIRCTORY_PATH
			+ "xwtoon-app-interface\\src\\main\\java\\com\\syswin\\xwtoon\\app\\service\\intro\\";
	private static final String ENTITY_INTERFACE_PATH = DIRCTORY_PATH
			+ "xwtoon-app-interface\\src\\main\\java\\com\\syswin\\xwtoon\\app\\service\\bean\\intro\\";
	...
	public GenDbCreateCodeForIntro() {
		// 初始化FreeMarker配置
		// 创建个Configuration实例
	...
	public static void main(String[] args) {
		try {
			GenDbCreateCodeForIntro hf = new GenDbCreateCodeForIntro();

			// 过滤点不用生成的表
			talbes = GenDb.getInstance().getTableName("qrtz");
			// hf.GenProcess(hf);
			hf.GenSingleProcess(hf, "org_intro"); // 指定单个数据库表生成代码;
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @author llh
	 */
	public void GenSingleProcess(GenDbCreateCodeForIntro hf, String tableName) throws Exception {
...
//    		 hf.buildTemplate(root,FTL_TEMPLATE_PATH,GenUtils.firstCharacterToUpper(className)+"_edit.ftl",pageForm,isOverwrite);
	 ...
	 * @author llh
	 */
	public void GenProcess(GenDbCreateCodeForIntro hf) throws Exception {
		Map<String, Object> root = new HashMap<String, Object>();
		setPageParm(root);
		Template template = cfg.getTemplate("entity.ftl");
		Template templateMapper = cfg.getTemplate("mapper.ftl");

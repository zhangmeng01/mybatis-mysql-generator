 ...
 * 自动代码生成工具操作类
 */
public class GenDbCreateCodeFordcdydao {

	private static Logger logger = LoggerFactory.getLogger(GenDbCreateCodeFordcdydao.class);
	// 项目的命名空间
	...
	public GenDbCreateCodeFordcdydao() {
		// 初始化FreeMarker配置
		// 创建个Configuration实例
	...
	public static void main(String[] args) {
		try {
			GenDbCreateCodeFordcdydao hf = new GenDbCreateCodeFordcdydao();
			// 过滤点不用生成的表
...
//		    hf.GenProcess(hf);
			 ...
			 */
			hf.GenSingleProcess("dcdy_message_content_manage"); // 指定单个数据库表生成代码;
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void GenSingleProcess(String tableName) throws Exception {
		...
		className = GenUtils.replaceUnderlineAndfirstToUpper(tableName.replace("dcdy", ""), "_", "");
...
//		 createController();
	 ...
	 * @author llh
	 */
	public void GenProcess(GenDbCreateCodeFordcdydao hf) throws Exception {
	...
	public void createDao() throws Exception {
		...
		GenDbCreateCodeFordcdydao.buildTemplate(root, ENTITY_PATH, GenUtils.firstCharacterToUpper(className) + ".java",
				template, isOverwrite);
		GenDbCreateCodeFordcdydao.buildTemplate(root, MAPPER_PATH, GenUtils.firstCharacterToUpper(className) + ".xml",
				templateMapper, isOverwrite);
		GenDbCreateCodeFordcdydao.buildTemplate(root, DAO_PATH,
	...
	public void createServiceEntity() throws Exception {
		...
		GenDbCreateCodeFordcdydao.buildTemplate(root, ENTITY_INTERFACE_PATH,
	...
	public void createServiceImpl() throws Exception {
		...
		root.put("package", PACKAGE_SERVICE);
		GenDbCreateCodeFordcdydao.buildTemplate(root, SERVICE_PATH,
				"I" + GenUtils.firstCharacterToUpper(className) + "Service.java", templateService, isOverwrite);
		GenDbCreateCodeFordcdydao.buildTemplate(root, SERVICEIMPL_PATH,
	...
	public void createController() throws Exception {
		...
		GenDbCreateCodeFordcdydao.buildTemplate(root, CONTROLLER_PATH,
		...
		GenDbCreateCodeFordcdydao.buildTemplate(root, FTL_TEMPLATE_PATH,
